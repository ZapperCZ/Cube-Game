...
public class Player_Movement : MonoBehaviour
{
    ...
    [SerializeField] Transform CeilingCheckParent;
    ...
    void Start()
    {
        ...
        CeilingCheckParent.localPosition = new Vector3(0,(defaultHeight-((difference)/2))/2,0);     //Places the cylinder between the maximum and minimum height of the character
        CeilingCheckParent.GetComponent<CylinderCollider>().cylinderHeight = difference;
        GroundCheckParent.GetComponent<TriggerChildManager>().CollisionMask = GroundMask;
        CeilingCheckParent.GetComponent<TriggerChildManager>().CollisionMask = LayerMask.NameToLayer("Default");
    ...
    void Update()
    {
        ...
        {
            if (!CeilingCheckParent.GetComponent<TriggerChildManager>().isTriggered)    //No obstruction above
            {
